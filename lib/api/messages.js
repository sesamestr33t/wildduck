...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Detect header-based searches (from/to/subject) to apply index hint
            // and skip expensive countDocuments
            let useHeaderHint = filter && filter.$and && filter.$and.some(clause =>
                (clause.headers && clause.headers.$elemMatch) ||
                (clause.$or && clause.$or.some(sub => sub.headers && sub.headers.$elemMatch))
            );

            // Skip countDocuments for header searches and $text searches — it runs
            // the same expensive scan a second time just to get a total count
            let isTextSearch = filter && filter.$text;
            let total = (useHeaderHint || isTextSearch) ? 0 : await getFilteredMessageCount(filter);
            ...
            let opts = {
                limit,
                query: filter,
                hint: useHeaderHint ? 'user_headers_search' : undefined,
                fields: {
                    ...
                    maxTimeMS: useHeaderHint ? 10 * 1000 : isTextSearch ? consts.DB_MAX_TIME_MESSAGES : consts.DB_MAX_TIME_MESSAGES_SEARCH
                },
