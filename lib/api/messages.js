...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Detect $text either at top level (prepareSearchFilter) or inside $and (getMongoDBQuery)
            let textClause = filter && filter.$text;
            if (!textClause && filter && filter.$and) {
                for (let entry of filter.$and) {
                    if (entry && entry.$text) {
                        textClause = entry.$text;
                        break;
                    }
                }
            }
            let isTextSearch = !!textClause;

            // Two-stage optimization: when multiple $and conditions are present,
            // MongoDB can only use one index. Pre-filter with the first condition,
            // get matching IDs, then apply remaining conditions constrained to those IDs.
            // For $text: replace with $regex on the text field since $text ignores $in constraints.
            let wasPreFiltered = false; // set when two-stage optimization was applied
            if (filter.$and && filter.$and.length) {
                ...
                // Pre-filter using the first indexed condition
                if (nonTextTerms.length) {
                    let preFilter = { user: filter.user };
                    if (filter.mailbox) {
                        preFilter.mailbox = filter.mailbox;
                    }
                    // Use first condition for pre-filter (most selective)
                    Object.assign(preFilter, nonTextTerms[0]);

                    let preResults = await db.database.collection('messages')
                        .find(preFilter, { projection: { _id: 1 } })
                        .maxTimeMS(consts.DB_MAX_TIME_MESSAGES)
                        .toArray();
                    ...
                    if (!preResults.length) {
                        isTextSearch = false;
                        wasPreFiltered = true;
                        filter = { _id: null }; // will match nothing
                    } else {
                        let ids = preResults.map(d => d._id);
                        wasPreFiltered = true;
                        // Rebuild filter: IDs + remaining conditions (skip the first, already applied)
                        let remainingTerms = nonTextTerms.slice(1);
                        filter = { user: filter.user, _id: { $in: ids } };
                        if (remainingTerms.length) {
                            filter.$and = remainingTerms;
                        }
                        // Replace $text with $regex on text field — $text ignores $in constraints
                        // but $regex on pre-filtered IDs is fast
                        if (textClause) {
                            filter.searchable = true;
                            filter.text = { $regex: tools.escapeRegexStr(textClause.$search), $options: 'i' };
                            isTextSearch = false; // no longer using $text
                        }
                    }
                }
            }

            // For $text, pre-filtered, and search-field queries, run count in parallel (non-blocking)
            // so it doesn't delay the response. Falls back to results.length if it times out.
            let hasSearchFilter = filter['search.subjectWords'] || filter['search.from'] || filter['search.to'] ||
                filter['search.cc'] || filter['search.bcc'] || filter['search.fromName'] || filter['search.toName'] ||
                filter['search.ccName'] || filter['search.bccName'] || filter['search.subject'];
            let totalPromise;
            if (isTextSearch || wasPreFiltered || hasSearchFilter) {
                totalPromise = db.database.collection('messages')
                    .countDocuments(filter, { maxTimeMS: consts.DB_MAX_TIME_MESSAGES })
                    .catch(() => 0);
            }
            let total = (isTextSearch || wasPreFiltered || hasSearchFilter) ? null : await getFilteredMessageCount(filter);
            ...
            // Resolve parallel $text count — use results length as fallback if it timed out
            if (totalPromise) {
                total = await totalPromise;
            }
            if (!total && listingWrapper.listing.results && listingWrapper.listing.results.length) {
                total = listingWrapper.listing.results.length;
            }
