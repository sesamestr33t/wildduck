...
module.exports = (db, server, messageHandler, userHandler, storageHandler, settingsHandler) => {
    ...
    server.get(
        ...
        tools.responseWrapper(async (req, res) => {
            ...
            // Skip countDocuments for header searches — it runs the same expensive
            // regex scan a second time just to get a total count
            let total = useHeaderHint ? 0 : await getFilteredMessageCount(filter);
            ...
            let opts = {
                ...
                fields: {
                    ...
                    maxTimeMS: useHeaderHint ? 60 * 1000 : consts.DB_MAX_TIME_MESSAGES_SEARCH
                },
