...
const prepareSearchFilter = async (db, user, payload) => {
    ...
    if (filterFrom) {
        let regex = escapeRegexStr(filterFrom.toLowerCase());
        if (!filter.$and) {
            filter.$and = [];
        }
        filter.$and.push({
    ...
    if (orTerms.from) {
        let regex = escapeRegexStr(orTerms.from.toLowerCase());
        orQuery.push({
            headers: {
                $elemMatch: {
                    key: 'from',
    ...
    if (filterTo) {
        let searchTerm = filterTo.toLowerCase();
        if (!filter.$and) {
            filter.$and = [];
        }
        if (/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(searchTerm)) {
            // Complete email address — uses tight index bounds (instant lookup)
            filter.$and.push({
                headers: {
                    $elemMatch: {
                        key: { $in: ['to', 'cc'] },
                        value: { $in: [searchTerm, '<' + searchTerm + '>'] }
                    }
                }
            });
        } else {
            // Partial search — regex fallback
            let regex = escapeRegexStr(searchTerm);
            filter.$and.push({
                headers: {
                    $elemMatch: {
                        key: { $in: ['to', 'cc'] },
    ...
    if (orTerms.to) {
        let searchTerm = orTerms.to.toLowerCase();
    ...
    if (filterSubject) {
        let regex = escapeRegexStr(filterSubject.toLowerCase());
        if (!filter.$and) {
            filter.$and = [];
        }
        filter.$and.push({
    ...
    if (orTerms.subject) {
        let regex = escapeRegexStr(orTerms.subject.toLowerCase());
        orQuery.push({
            headers: {
                $elemMatch: {
                    key: 'subject',
