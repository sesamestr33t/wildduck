...
const prepareSearchFilter = async (db, user, payload) => {
    ...
    if (filterTo) {
        ...
        if (term.includes('@')) {
            filter.$and.push({ $or: [{ 'search.to': term }, { 'search.cc': term }, { 'search.bcc': term }] });
        } else {
            let regex = escapeRegexStr(term);
            filter.$and.push({ $or: [{ 'search.toName': { $regex: regex } }, { 'search.ccName': { $regex: regex } }, { 'search.bccName': { $regex: regex } }] });
        }
    }
    ...
    if (orTerms.to) {
        ...
        if (term.includes('@')) {
            orQuery.push({ $or: [{ 'search.to': term }, { 'search.cc': term }, { 'search.bcc': term }] });
        } else {
            let regex = escapeRegexStr(term);
            orQuery.push({ $or: [{ 'search.toName': { $regex: regex } }, { 'search.ccName': { $regex: regex } }, { 'search.bccName': { $regex: regex } }] });
        }
    }

    if (filterSubject) {
        let term = filterSubject.toLowerCase().trim();
        let words = term.split(/[\s,.;:!?()[\]{}<>"'`~/\\|@#$%^&*+=_-]+/).filter(w => w.length > 0);
        if (words.length) {
            // Use $all to match all words as exact elements in the array (efficient with array indexes)
            filter['search.subjectWords'] = { $all: words };
        }
    }
    ...
    if (orTerms.subject) {
        ...
        if (words.length) {
            // Use $all to match all words as exact elements in the array (efficient with array indexes)
            orQuery.push({ 'search.subjectWords': { $all: words } });
        }
    }
