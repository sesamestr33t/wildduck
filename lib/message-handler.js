...
class MessageHandler {
    ...
    buildSearchFields(mimeTree) {
        const parsedHeader = mimeTree.parsedHeader || {};
        const search = {};

        const extractAddressesAndNames = addrs => {
            const addresses = addrs.map(a => (a.address || '').toString().toLowerCase().trim()).filter(a => a && a.includes('@'));
            const names = addrs.map(a => (a.name || '').toString().toLowerCase().trim()).filter(n => n);
            return { addresses, names };
        };

        // Extract from addresses and name
        const { addresses: fromAddresses, names: fromNames } = extractAddressesAndNames([].concat(parsedHeader.from || []));
        if (fromAddresses.length) search.from = fromAddresses;
        if (fromNames.length) search.fromName = fromNames.join(' ');

        // Extract to addresses and names
        const { addresses: toAddresses, names: toNames } = extractAddressesAndNames([].concat(parsedHeader.to || []));
        if (toAddresses.length) search.to = toAddresses;
        if (toNames.length) search.toName = toNames.join(' ');

        // Extract cc addresses and names
        const { addresses: ccAddresses, names: ccNames } = extractAddressesAndNames([].concat(parsedHeader.cc || []));
        if (ccAddresses.length) search.cc = ccAddresses;
        if (ccNames.length) search.ccName = ccNames.join(' ');

        // Extract bcc addresses and names (preserved in sender's Sent folder)
        const { addresses: bccAddresses, names: bccNames } = extractAddressesAndNames([].concat(parsedHeader.bcc || []));
        if (bccAddresses.length) search.bcc = bccAddresses;
        if (bccNames.length) search.bccName = bccNames.join(' ');

        // Extract subject — store both full string and tokenized words array
        const subject = ([].concat(parsedHeader.subject || []).pop() || '').toString();
        if (subject) {
            let decoded = subject;
            try {
                decoded = libmime.decodeWords(subject);
            } catch (e) {
                // ignore
            }
            let subjectLower = decoded.toLowerCase().trim();
            search.subject = subjectLower;
            // Tokenize into words for fast indexed prefix matching
            let words = subjectLower.split(/[\s,.;:!?()[\]{}<>"'`~/\\|@#$%^&*+=_-]+/).filter(w => w.length > 0);
            if (words.length) {
                search.subjectWords = [...new Set(words)];
            }
        }
    ...
    async prepareMessageAsync(options) {
        ...
        let search = this.buildSearchFields(mimeTree);
