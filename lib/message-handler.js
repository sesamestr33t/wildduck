...
class MessageHandler {
    ...
    buildSearchFields(mimeTree) {
        ...
        const fromAddrs = [].concat(parsedHeader.from || []);
        if (fromAddrs.length) {
            search.from = fromAddrs
                .map(a => (a.address || '').toString().toLowerCase().trim())
                .filter(a => a && a.includes('@'));
            const fromNames = fromAddrs
                .map(a => (a.name || '').toString().toLowerCase().trim())
                .filter(n => n);
            if (fromNames.length) {
                search.fromName = fromNames.join(' ');
            }
        }

        // Extract to addresses and names
        const toAddrs = [].concat(parsedHeader.to || []);
        if (toAddrs.length) {
            search.to = toAddrs
                .map(a => (a.address || '').toString().toLowerCase().trim())
                .filter(a => a && a.includes('@'));
            const toNames = toAddrs
                .map(a => (a.name || '').toString().toLowerCase().trim())
                .filter(n => n);
            if (toNames.length) {
                search.toName = toNames.join(' ');
            }
        }

        // Extract cc addresses and names
        const ccAddrs = [].concat(parsedHeader.cc || []);
        if (ccAddrs.length) {
            search.cc = ccAddrs
                .map(a => (a.address || '').toString().toLowerCase().trim())
                .filter(a => a && a.includes('@'));
            const ccNames = ccAddrs
                .map(a => (a.name || '').toString().toLowerCase().trim())
                .filter(n => n);
            if (ccNames.length) {
                search.ccName = ccNames.join(' ');
            }
        }
