                    ...
                    case 'from':
                        {
                            ...
                            let branch;
                            if (term.includes('@')) {
                                branch = { 'search.from': term };
                            } else {
                                branch = { 'search.fromName': { $regex: escapeRegexStr(term) } };
                            }
                            if (negated) {
                                branch = { $nor: [branch] };
                            }
                    ...
                    case 'subject':
                        {
                            ...
                            let branch;
                            if (words.length === 1) {
                                branch = { 'search.subjectWords': { $regex: '^' + escapeRegexStr(words[0]) } };
                            } else {
                                branch = { $and: words.map(w => ({ 'search.subjectWords': { $regex: '^' + escapeRegexStr(w) } })) };
                            }
                            if (negated) {
                                branch = { $nor: [branch] };
                            }
                    ...
                    case 'to':
                        {
                            ...
                            let branch;
                            if (term.includes('@')) {
                                branch = { $or: [{ 'search.to': term }, { 'search.cc': term }, { 'search.bcc': term }] };
                            } else {
                                branch = { $or: [{ 'search.toName': { $regex: escapeRegexStr(term) } }, { 'search.ccName': { $regex: escapeRegexStr(term) } }, { 'search.bccName': { $regex: escapeRegexStr(term) } }] };
                            }
                            if (negated) {
                                branch = { $nor: [branch] };
                            }
