                        ...
                        case 'header':
                            {
                                ...
                                if (['from', 'to', 'cc', 'bcc', 'subject'].includes(term.header) && searchValue) {
                                    ...
                                    if (term.header === 'from') {
                                        ...
                                        let words = searchValue.split(/\s+/).filter(w => w);
                                        if (words.length === 1) {
                                            entry = { 'search.subjectWords': { $regex: '^' + tools.escapeRegexStr(words[0]) } };
                                        } else {
                                            entry = { $and: words.map(w => ({ 'search.subjectWords': { $regex: '^' + tools.escapeRegexStr(w) } })) };
                                        }
                                    }
                                    if (ne) {
                                        entry = { $nor: [entry] };
