            ...
            let walkQuery = (parent, ne, node) => {
                ...
                node.forEach(term => {
                    switch (term.key) {
                        ...
                        case 'header':
                            {
                                let searchValue = Buffer.from(term.value, 'binary').toString().toLowerCase().trim();
                                let entry;

                                if (['from', 'to', 'cc', 'bcc', 'subject'].includes(term.header) && searchValue) {
                                    // Use parsed search fields for fast lookup
                                    if (term.header === 'from') {
                                        entry = searchValue.includes('@')
                                            ? { 'search.from': searchValue }
                                            : { 'search.fromName': { $regex: tools.escapeRegexStr(searchValue) } };
                                    } else if (term.header === 'to' || term.header === 'cc' || term.header === 'bcc') {
                                        entry = searchValue.includes('@')
                                            ? { ['search.' + term.header]: searchValue }
                                            : { ['search.' + term.header + 'Name']: { $regex: tools.escapeRegexStr(searchValue) } };
                                    } else if (term.header === 'subject') {
                                        let words = searchValue.split(/[\s,.;:!?()[\]{}<>"'`~/\\|@#$%^&*+=_-]+/).filter(w => w.length > 0);
                                        if (words.length) {
                                            // Use $all to match all words as exact elements in the array (efficient with array indexes)
                                            entry = { 'search.subjectWords': { $all: words } };
                                        }
                                    }
                                    if (ne) {
                                        entry = { $nor: [entry] };
                                    ...
                                    // Keep existing behavior for other headers
